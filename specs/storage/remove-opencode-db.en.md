# Remove `packages/miaopan-code/src/storage/db.ts`

Languages: [简体中文](remove-opencode-db.md) · English

## Goal

Remove all production usages of the legacy `packages/miaopan-code/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the miaopan-code legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/miaopan-code/src/storage/db.ts` are concentrated in 22 source files:
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy miaopan-code database wrapper; `packages/miaopan-code/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the miaopan-code wrapper.
...
Status: Completed. `SyncEvent` and the miaopan-code projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy miaopan-code database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy miaopan-code database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy miaopan-code database wrapper.
...
All migration groups are complete or superseded. `packages/miaopan-code/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No miaopan-code data-migration group remains.

The previous miaopan-code `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate miaopan-code data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/miaopan-code`.
...
- `bun typecheck` from `packages/miaopan-code`
- Relevant package tests from `packages/miaopan-code`, not the repo root
