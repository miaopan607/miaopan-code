...
Move the canonical miaopan-code terminal application from
`packages/miaopan-code/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @miaopan-code/tui
```

Target dependency graph:

```text
packages/miaopan-code ---\
                      > @miaopan-code/tui -> @miaopan-code/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/miaopan-code`,
`packages/cli`, or `@miaopan-code/core`.

The SDK is the TUI's miaopan-code boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/miaopan-code`.
- Do not replace private `packages/miaopan-code` imports with `@miaopan-code/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@miaopan-code/tui`.

## Ownership Boundary

### `@miaopan-code/tui` Owns
...
- miaopan-code domain data displayed by the TUI
...
```text
packages/miaopan-code/src/cli/cmd/tui
```

Its private dependency on `packages/miaopan-code` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/miaopan-code/src/*`.
...
Status: Completed. The private `@miaopan-code/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@miaopan-code/tui`.
...
- The package has no dependency on `miaopan-code`, `@miaopan-code/cli`, or
  `@miaopan-code/core`.
...
primitives now live in `@miaopan-code/tui`. The legacy host consumes explicit
...
and build inputs through `@miaopan-code/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads miaopan-code globals or process state;
...
Replace process-global miaopan-code state with resolved TUI inputs.
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/miaopan-code` adapters.
...
`@miaopan-code/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/miaopan-code` initially.
...
canonical in `@miaopan-code/tui`. Configured references resolve through the new
...
adapters rather than miaopan-code domain implementations.

Make the SDK the only miaopan-code domain boundary used by the TUI.
...
- Domain-facing TUI code imports miaopan-code data and operations only from
  `@miaopan-code/sdk`.
...
`@miaopan-code/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/miaopan-code`.
...
- The package has no imports from `packages/miaopan-code`, `packages/cli`, or
  `@miaopan-code/core`.
...
public `@miaopan-code/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/miaopan-code`.
- Keep the legacy embedded worker and server startup in `packages/miaopan-code`.
- Change those adapters to load config, create transport inputs, and call the
  public `@miaopan-code/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/miaopan-code`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@miaopan-code/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@miaopan-code/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for miaopan-code domain state.
...
cd packages/miaopan-code && bun typecheck
...
rg '@miaopan-code/core|packages/miaopan-code|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/miaopan-code/src packages/miaopan-code/test
```
...
- Build the current-platform `packages/miaopan-code` binary.
