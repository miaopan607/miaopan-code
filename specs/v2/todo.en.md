...
Languages: [简体中文](todo.md) · English

ok we need to work towards a launch of v2 so we can get out of this rebuild phase

## Post-Hono cleanup - Kit

The miaopan-code server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@miaopan-code/llm` tool loop after replacing its
...
- plugins get global "miaopan-code" instance like in that post i showed
- miaopan-code instance has stuff like `miaopan-code.session.prompt()` or
  `miaopan-code.tool.register({...})`
...
- decide whether to preserve deprecated `@miaopan-code/llm` orchestration exports
