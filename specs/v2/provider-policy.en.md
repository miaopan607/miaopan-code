...
Languages: [简体中文](provider-policy.md) · English
...
Both `action` and `resource` use miaopan-code's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.miaopan-code` files is intentionally deferred until `.miaopan-code` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether miaopan-code permits an operation through its managed execution paths.
