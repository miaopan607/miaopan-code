...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".miaopanCode/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".miaopanCode/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".miaopanCode/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "miaopan-code/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
  ...
  test("resolves variants from verbose model output", () => {
    const output = `miaopan-code/other
{"variants":{}}
miaopanCode/gpt-5.5
...
{"variants":{}}
`
    expect(modelVariants(output, "miaopan-code/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "miaopan-code/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
