...
const model = "miaopan-code/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/miaopan-code")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with miaopanCode...`)
  ...
  try {
    await $`miaopan-code run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  miaopanCode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with miaopanCode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with miaopanCode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/miaopan-code`.",
