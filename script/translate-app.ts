...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "miaopan-code/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".miaopanCode/glossary/zh-cn.md"
  if (locale === "zht") return ".miaopanCode/glossary/zh-tw.md"
  return `.miaopanCode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("MiaopanCode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("MiaopanCode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("MiaopanCode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel MiaopanCode runs for 'all' (default: 4)
      --model <provider/id>  MiaopanCode model (default: miaopanCode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running MiaopanCode
...
`)
  ...
  if (failed.length) console.error(`\nMiaopanCode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.MIAOPAN_CODE_DISABLE_PROJECT_CONFIG = "1"
  env.MIAOPAN_CODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["miaopan-code", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.MIAOPAN_CODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["miaopan-code", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.MIAOPAN_CODE_CONFIG
  delete env.MIAOPAN_CODE_CONFIG_DIR
  delete env.MIAOPAN_CODE_CONFIG_CONTENT
  delete env.MIAOPAN_CODE_PERMISSION
  delete env.MIAOPAN_CODE_AUTO_SHARE
  return env
}
