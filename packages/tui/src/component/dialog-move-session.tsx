...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current)
      return [{ title: tr("workspace.loading_directories"), value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: tr("workspace.no_directories"), value: undefined }]
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>
            {t(Locale.language(), "workspace.deleting_path", { path: item.location })}
          </span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>
            {t(Locale.language(), "workspace.delete_confirm_again", { key: deleteHint() })}
          </span>
        ) : suffix ? (
          <>
        ...
        category: item.root.directory === current ? tr("cli.run.current") : tr("provider.other"),
        titleWidth,
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: tr("workspace.delete_copy_title"),
          message: tr("workspace.delete_copy_message"),
        })
        if (choice !== "yes") {
          reopen()
          return
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: tr("workspace.delete_copy_failed"),
  ...
  return (
    <box minHeight={showError() ? 5 : fullHeight()}>
      <DialogSelect
        title={tr("workspace.move_session")}
        titleView={
          <box flexDirection="row" gap={1}>
            <text fg={theme.text} attributes={TextAttributes.BOLD}>
              {tr("workspace.move_session")}
            </text>
