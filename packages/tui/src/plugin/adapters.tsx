import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@miaopan-code/plugin/tui"
...
type Input = {
  ...
  keymap: ReturnType<typeof useMiaopanCodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getMiaopanCodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getMiaopanCodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
