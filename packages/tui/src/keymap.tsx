...
export const MIAOPAN_CODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const MIAOPAN_CODE_MODE_KEY = "miaopanCode.mode"

export const MiaopanCodeKeymapProvider = KeymapProvider
export const useMiaopanCodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type MiaopanCodeModeStack = ReturnType<typeof createMiaopanCodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, MiaopanCodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createMiaopanCodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(MIAOPAN_CODE_MODE_KEY, MIAOPAN_CODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(MIAOPAN_CODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(MIAOPAN_CODE_MODE_KEY, stack.at(-1)?.mode ?? MIAOPAN_CODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? MIAOPAN_CODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(MIAOPAN_CODE_MODE_KEY, undefined)
...
export function useMiaopanCodeModeStack() {
  return getMiaopanCodeModeStack(useMiaopanCodeKeymap())
}
...
export function registerMiaopanCodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createMiaopanCodeModeStack(keymap)
