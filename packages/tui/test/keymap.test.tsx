...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerMiaopanCodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when miaopanCode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getMiaopanCodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getMiaopanCodeModeStack(keymap).push("autocomplete")
