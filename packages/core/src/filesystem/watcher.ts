...
declare const MIAOPAN_CODE_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof MIAOPAN_CODE_LIBC === "undefined" ? undefined : MIAOPAN_CODE_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@miaopan-code/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.MIAOPAN_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.MIAOPAN_CODE_EXPERIMENTAL_FILEWATCHER)) {
