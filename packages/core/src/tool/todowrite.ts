...
export const Input = Schema.Struct({
  todos: Schema.Array(SessionTodo.Info).annotate({ description: zh("tool.param.core_todo") }),
})
...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* tools
      .register({
        [name]: Tool.make({
          description: zh("tool.description.core_todowrite"),
          input: Input,
          output: Output,
          ...
          execute: (input, context) =>
            ...
            }).pipe(Effect.mapError(() => new ToolFailure({ message: t(context.language, "tool.error.todos") }))),
        }),
      })
      .pipe(Effect.orDie)
  }),
