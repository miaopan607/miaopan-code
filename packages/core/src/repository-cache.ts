...
export class Service extends Context.Service<Service, Interface>()("@miaopan-code/RepositoryCache") {}
          ...
          return yield* flock
            .withLock(
              Effect.gen(function* () {
                ...
                if (status === "refreshed") {
                  if (!existing)
                    return yield* new FetchFailedError({ repository, message: zh("error.repository_unavailable") })
                  yield* git.sync
