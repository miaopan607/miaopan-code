...
const miaopanCodeClient = process.env.MIAOPAN_CODE_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (miaopanCodeClient === undefined) delete process.env.MIAOPAN_CODE_CLIENT
  else process.env.MIAOPAN_CODE_CLIENT = miaopanCodeClient
})

describe("resource", () => {
  test("parses and decodes OTEL resource attributes", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "service.namespace=miaopan-code,team=platform%2Cobservability,label=hello%3Dworld,key%2Fname=value%20here"

    expect(resource().attributes).toMatchObject({
      "service.namespace": "miaopan-code",
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES = "service.namespace=miaopan-code,broken"

    expect(resource().attributes["service.namespace"]).toBeUndefined()
    expect(resource().attributes["miaopanCode.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.MIAOPAN_CODE_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "miaopanCode.client=web,service.instance.id=override,service.namespace=miaopan-code"

    expect(resource().attributes).toMatchObject({
      "miaopanCode.client": "cli",
      "service.namespace": "miaopan-code",
    })
    expect(resource().attributes["service.instance.id"]).not.toBe("override")
    expect(resource().attributes["miaopanCode.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "miaopanCode-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "miaopanCode.log")
