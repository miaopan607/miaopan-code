...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "miaopanCode-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["MIAOPAN_CODE_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { MIAOPAN_CODE_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("MIAOPAN_CODE_TEST_SHELL=ok")
      }),
    )
