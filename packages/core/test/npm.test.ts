...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@miaopan-code/acme")).toBe("@miaopan-code/acme")
    expect(Npm.sanitize("@miaopan-code/acme@1.0.0")).toBe("@miaopan-code/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/miaopanCode/acme.git"
    const expected = win ? "acme@git+https_//github.com/miaopanCode/acme.git" : spec
