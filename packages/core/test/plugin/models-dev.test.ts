...
import { ModelsDevPlugin } from "@miaopan-code/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.MIAOPAN_CODE_MODELS_PATH,
          disabled: Flag.MIAOPAN_CODE_DISABLE_MODELS_FETCH,
        }
        Flag.MIAOPAN_CODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.MIAOPAN_CODE_MODELS_PATH = previous.path
          Flag.MIAOPAN_CODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
