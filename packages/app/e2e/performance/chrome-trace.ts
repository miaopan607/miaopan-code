...
export async function startChromeTrace(page: Page, name: string) {
  const directory = process.env.MIAOPAN_CODE_PERFORMANCE_TRACE_DIR
  if (!directory) return

  const selectors = process.env.MIAOPAN_CODE_PERFORMANCE_SELECTOR_TRACE === "1"
...
export async function prepareChromeTrace(
  ...
  const run = process.env.MIAOPAN_CODE_PERFORMANCE_RUN_ID ?? "manual"
