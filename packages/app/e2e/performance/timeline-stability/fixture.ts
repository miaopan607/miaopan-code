...
import { SessionStatusEvent } from "@miaopan-code/schema/session-status-event"
import { SessionV1 } from "@miaopan-code/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockMiaopanCodeServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/MiaopanCode/TimelineStability"
...
export const model = { providerID: "miaopan-code", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  await mockMiaopanCodeServer(page, {
    directory,
    project: project(),
    provider: provider(),
    sessions,
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("miaopanCode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["miaopan-code"],
    default: { providerID: "miaopan-code", modelID: "claude-opus-4-6" },
  }
}
