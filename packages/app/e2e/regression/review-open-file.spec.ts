...
const directory = "C:/MiaopanCode/ReviewOpenFile"
...
test("opens and searches project files inline", async ({ page }) => {
  ...
  await mockMiaopanCodeServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "miaopan-code", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "miaopanCode.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "miaopanCode.window.browser.dat:tabs",
