...
import { type Session } from "@miaopan-code/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("miaopanCode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("miaopanCode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("miaopanCode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("miaopanCode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("miaopanCode://open-project")).toBeUndefined()
    expect(parseDeepLink("miaopanCode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "miaopanCode://open-project?directory=/a",
      "miaopanCode://other?directory=/b",
      "miaopanCode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("miaopanCode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("miaopanCode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("miaopanCode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("miaopanCode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "miaopanCode://new-session?directory=/a",
      "miaopanCode://open-project?directory=/b",
      "miaopanCode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MIAOPAN_CODE__: {
        deepLinks: ["miaopanCode://open-project?directory=/a"],
      },
    } as unknown as Window & { __MIAOPAN_CODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["miaopanCode://open-project?directory=/a"])
