...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when MiaopanCode needs attention", () => {
    expect(wslMiaopanCodeAction(undefined)).toBeUndefined()
    expect(
      wslMiaopanCodeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install MiaopanCode")
    expect(
      wslMiaopanCodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/miaopanCode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update MiaopanCode")
    expect(
      wslMiaopanCodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/miaopanCode",
  ...
  test("does not report ready when MiaopanCode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        miaopanCodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.miaopanCode/bin/miaopanCode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installMiaopanCode" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-miaopanCode")
  })
