...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-miaopanCode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslMiaopanCodeAction(check?: WslMiaopanCodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install MiaopanCode"
  if (check.matchesDesktop === false) return "Update MiaopanCode"
}
...
export function addServerViewModel(input: {
  ...
  const miaopanCodeCheck = selectedDistro ? (state?.miaopanCodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.miaopanCodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateMiaopanCode" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.miaopanCodeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installMiaopanCode" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  miaopanCodeCheck: WslMiaopanCodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingMiaopanCode =
    probingSelected ||
    (ready &&
      (!input.miaopanCodeCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingMiaopanCode =
    input.state?.job?.kind === "install-miaopanCode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingMiaopanCode) {
    return {
      variant: "contrast",
  ...
  if (!addServerMiaopanCodeReady(input.miaopanCodeCheck)) {
    const update = !!input.miaopanCodeCheck?.resolvedPath && input.miaopanCodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingMiaopanCode
        ? { key: "wsl.onboarding.updatingMiaopanCode" }
        : update
          ? { key: "wsl.onboarding.updateMiaopanCode" }
          : { key: "wsl.onboarding.installMiaopanCode" },
      disabled: !!input.state?.job || input.adding,
      action: "install-miaopanCode",
      loading: installingMiaopanCode,
...
function addServerMiaopanCodeReady(check: WslMiaopanCodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.miaopanCodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.miaopanCodeChecks[item.name]) return [`miaopanCode:${item.name}`]
    return []
  })
