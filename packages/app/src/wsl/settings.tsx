...
import { wslMiaopanCodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.miaopanCodeChecks[item.config.distro]
          const miaopanCodeAction = () => wslMiaopanCodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-miaopanCode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={miaopanCodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installMiaopanCode(item.config.distro))}
                    >
