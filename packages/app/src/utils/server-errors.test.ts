...
import type { SessionNotFoundError } from "@miaopan-code/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em miaopanCode.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
