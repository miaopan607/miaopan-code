...
import { createMiaopanCode, type ToolPart } from "@miaopan-code/sdk"
...
console.log("🚀 Starting miaopanCode server...")
const miaopanCode = await createMiaopanCode({
  port: 0,
})
console.log("✅ MiaopanCode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await miaopanCode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new miaopanCode session...")
    const { client, server } = miaopanCode
    ...
    console.log("✅ Created miaopanCode session:", createResult.data.id)
  ...
  console.log("📝 Sending to miaopanCode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 MiaopanCode response:", JSON.stringify(result, null, 2))
