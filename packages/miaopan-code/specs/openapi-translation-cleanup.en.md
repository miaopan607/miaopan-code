...
Languages: [简体中文概览](openapi-translation-cleanup.md) · English

## Goal

Trim `packages/miaopan-code/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/miaopan-code/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/miaopan-code`.
- `[x]` `bun typecheck` from `packages/miaopan-code`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/miaopan-code/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/miaopan-code`.
- `[x]` `bun dev generate > /tmp/miaopan-code-openapi.json` from `packages/miaopan-code`.
...
- Edit `packages/miaopan-code/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/miaopan-code-openapi.json` from `packages/miaopan-code`.
...
- Check source schemas in files like `packages/miaopan-code/src/session/schema.ts`, `packages/miaopan-code/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/miaopan-code/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/miaopan-code/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
