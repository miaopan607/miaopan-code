...
```json
{
  "$schema": "https://github.com/miaopan607/miaopan-code/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/miaopan-code-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
    ...
    "sound": true,
    "volume": 0.4,
    "sound_pack": "miaopan-code.default",
    "sounds": {
...
- The value is a semver range checked against the running miaopan-code version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.miaopan-code` is absent, no check is performed (backward compatible).
...
- `miaopan-code plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `miaopan-code plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.miaopan-code` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.miaopan-code`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.miaopan-code`) is covered by regression tests.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus miaopan-code's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
miaopan-code registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, miaopan-code pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- `message` is required; `title` defaults to `"miaopan-code"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `miaopan-code.default`.
...
- Local plugins persist installed themes under the local `.miaopan-code/themes` area near the plugin config source.
...
- `--pure` / `MIAOPAN_CODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.miaopan-code/plugins/tui-smoke.tsx`
- Local vim plugin: `.miaopan-code/plugins/tui-vim.tsx`
- Local smoke config: `.miaopan-code/tui.json`
- Local smoke theme: `.miaopan-code/plugins/smoke-theme.json`
