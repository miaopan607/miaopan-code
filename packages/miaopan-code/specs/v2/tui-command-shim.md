...
Delete `packages/miaopan-code/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/miaopan-code/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/miaopan-code/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/miaopan-code`
- TUI plugin loader tests in `packages/miaopan-code` if runtime plugin API wiring changed
