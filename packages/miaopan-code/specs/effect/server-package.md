...
- The server still lives in `packages/miaopan-code`.
...
Until enough shared service code lives outside `packages/miaopan-code`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/miaopan-code`

It should not import `packages/miaopan-code` services while `packages/miaopan-code`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on miaopan-code-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/miaopan-code` implementation details.
