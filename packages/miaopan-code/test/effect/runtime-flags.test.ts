...
describe("RuntimeFlags", () => {
  ...
  it.effect("layer parses MIAOPAN_CODE_EXPERIMENTAL_LSP_TY", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            MIAOPAN_CODE_EXPERIMENTAL_LSP_TY: "true",
          }),
        ),
      )
  ...
  it.effect("disableExternalSkills reads MIAOPAN_CODE_DISABLE_EXTERNAL_SKILLS", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ MIAOPAN_CODE_DISABLE_EXTERNAL_SKILLS: "true" })))
  ...
  it.effect("disableLspDownload reads MIAOPAN_CODE_DISABLE_LSP_DOWNLOAD", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ MIAOPAN_CODE_DISABLE_LSP_DOWNLOAD: "true" })))
  ...
  it.effect("disableClaudeCodePrompt inherits MIAOPAN_CODE_DISABLE_CLAUDE_CODE", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ MIAOPAN_CODE_DISABLE_CLAUDE_CODE: "true" })))
  ...
  it.effect("experimentalIconDiscovery inherits MIAOPAN_CODE_EXPERIMENTAL", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ MIAOPAN_CODE_EXPERIMENTAL: "true" })))

      expect(flags.experimentalIconDiscovery).toBe(true)
    }),
  )

  it.effect("specific experimental flags override MIAOPAN_CODE_EXPERIMENTAL", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            MIAOPAN_CODE_EXPERIMENTAL: "true",
            MIAOPAN_CODE_EXPERIMENTAL_ICON_DISCOVERY: "false",
          }),
        ),
      )
  ...
  it.effect("experimentalOxfmt is enabled by MIAOPAN_CODE_EXPERIMENTAL_OXFMT", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
            MIAOPAN_CODE_EXPERIMENTAL_OXFMT: "true",
          }),
        ),
      )
  ...
  it.effect("experimentalOxfmt inherits MIAOPAN_CODE_EXPERIMENTAL", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          fromConfig({
  ...
  for (const input of [
    { name: "absent", config: {}, expected: undefined },
    {
      ...
      config: { MIAOPAN_CODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "1234" },
      expected: 1234,
    },
    {
      name: "invalid string",
      config: { MIAOPAN_CODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "nope" },
      expected: undefined,
    },
    { name: "zero", config: { MIAOPAN_CODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "0" }, expected: undefined },
    { name: "negative", config: { MIAOPAN_CODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "-1" }, expected: undefined },
    {
      name: "non-integer",
      config: { MIAOPAN_CODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "1.5" },
      expected: undefined,
    },
  ]) {
  ...
  for (const input of [
    ...
    {
      name: "valid positive integer",
      config: { MIAOPAN_CODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "1234" },
      expected: 1234,
    },
    {
      name: "invalid string",
      config: { MIAOPAN_CODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "nope" },
      expected: undefined,
    },
    { name: "zero", config: { MIAOPAN_CODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "0" }, expected: undefined },
    { name: "negative", config: { MIAOPAN_CODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "-1" }, expected: undefined },
    {
      name: "non-integer",
      config: { MIAOPAN_CODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "1.5" },
      expected: undefined,
    },
  ]) {
  ...
  it.effect("disableClaudeCodeSkills inherits MIAOPAN_CODE_DISABLE_CLAUDE_CODE", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(Effect.provide(fromConfig({ MIAOPAN_CODE_DISABLE_CLAUDE_CODE: "true" })))
