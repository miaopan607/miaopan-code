// Subprocess test harness for the miaopanCode CLI. Spawns the real binary against
...
//
// Configuration flows through miaopanCode's built-in test affordances:
//   - MIAOPAN_CODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - MIAOPAN_CODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - MIAOPAN_CODE_DISABLE_PROJECT_CONFIG : skip walking up for miaopan-code.json
//   - MIAOPAN_CODE_PURE                : skip external plugin discovery + install
//   - MIAOPAN_CODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `miaopanCode.run` is fully wired. The shape supports adding more
// builders (`miaopanCode.serve(opts)`, `miaopanCode.acp(opts)`, `miaopanCode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of MiaopanCodeCli.
...
const miaopanCodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(miaopanCodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    MIAOPAN_CODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    MIAOPAN_CODE_CONFIG_CONTENT: configJson,
    MIAOPAN_CODE_DISABLE_PROJECT_CONFIG: "1",
    MIAOPAN_CODE_PURE: "1",
    MIAOPAN_CODE_DISABLE_AUTOUPDATE: "1",
    MIAOPAN_CODE_DISABLE_AUTOCOMPACT: "1",
    MIAOPAN_CODE_DISABLE_MODELS_FETCH: "1",
    MIAOPAN_CODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `miaopan-code run`. New flags should
...
// `miaopan-code serve` is a long-lived process — it never exits on its own.
...
// `miaopanCode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type MiaopanCodeCli = {
  ...
  // Spawn `miaopan-code serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `miaopanCode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly miaopanCode: MiaopanCodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("miaopanCode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          MIAOPAN_CODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("miaopanCode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("miaopanCode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      yield* Effect.forkScoped(
        fromBunStream("stdout", () => proc.stdout).pipe(
          ...
          Stream.runForEach((line) => {
            ...
            return m ? Deferred.succeed(readyDeferred, { url: m[0], hostname: m[1], port: Number(m[2]) }) : Effect.void
          }),
          Effect.ignore({ log: true }),
        ),
      )
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `miaopan-code serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("miaopanCode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const miaopanCode: MiaopanCodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, miaopanCode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `miaopanCode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "miaopan-code") {
...
// every call site. Use this for any test that needs the miaopanCode CLI fixture.
//
...
// resources (e.g. `miaopanCode.serve`) without an extra `Effect.scoped` wrapper —
