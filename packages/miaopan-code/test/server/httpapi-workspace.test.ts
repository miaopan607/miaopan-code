...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-miaopanCode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-miaopanCode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://github.com/miaopan607/miaopan-code/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-miaopanCode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-miaopanCode-workspace")
