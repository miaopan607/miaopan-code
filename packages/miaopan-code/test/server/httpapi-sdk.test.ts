...
type Sdk = ReturnType<typeof createMiaopanCodeClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.MIAOPAN_CODE_SERVER_PASSWORD = input?.password
      Flag.MIAOPAN_CODE_SERVER_USERNAME = input?.username
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-miaopanCode-directory")).toBe(false)
        expect(request!.headers.has("x-miaopanCode-workspace")).toBe(false)
      }),
    ),
  )
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("miaopan-code", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("miaopan-code", "secret") },
        })
