...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips miaopanCode-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("http://localhost/test", {
          headers: {
            ...
            "x-miaopanCode-workspace": "ws_123",
            "x-custom": "preserved",
          },
        }),
      )
      ...
      expect(forwarded["x-miaopanCode-directory"]).toBeUndefined()
      expect(forwarded["x-miaopanCode-workspace"]).toBeUndefined()
