...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "miaopan-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "miaopan-code.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "miaopan-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "miaopan-code.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["miaopan-code.json", "miaopan-code.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "miaopan-code.json"),
        path.join(project, "miaopan-code.jsonc"),
      ])
    })
  })
