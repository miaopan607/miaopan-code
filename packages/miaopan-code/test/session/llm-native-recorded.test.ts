...
import { HttpRecorder } from "@miaopan-code/http-recorder"
import { HttpRecorderInternal } from "@miaopan-code/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@miaopan-code/llm"
import { RequestExecutor } from "@miaopan-code/llm/route"
...
const zenURL = (connection: string) => `https://console.miaopanCode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.MIAOPAN_CODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["miaopan-code", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MIAOPAN_CODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("MIAOPAN_CODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["miaopan-code", "native", "oauth", "tool-loop"],
  ...
  {
    id: "miaopanCode-proxy",
    name: "MiaopanCode proxy",
    providerID: ProviderV2.ID.miaopanCode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["miaopan-code", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.MIAOPAN_CODE_RECORD_CONSOLE_TOKEN && process.env.MIAOPAN_CODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.miaopanCode,
        name: "MiaopanCode Zen",
        env: ["MIAOPAN_CODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.MIAOPAN_CODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.MIAOPAN_CODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.MIAOPAN_CODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["miaopan-code", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MIAOPAN_CODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("MIAOPAN_CODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("MIAOPAN_CODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set MIAOPAN_CODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the miaopanCode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "miaopan-code.json"),
      JSON.stringify({ $schema: "https://github.com/miaopan607/miaopan-code/config.json", ...scenario.config(model) }),
    ),
  )
