...
type SessionMessage = NonNullable<Awaited<ReturnType<MiaopanCodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<MiaopanCodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<MiaopanCodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://miaopanCode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: MiaopanCodeClient["event"]["subscribe"]
    globalEvent?: MiaopanCodeClient["global"]["event"]
    promptAsync?: MiaopanCodeClient["session"]["promptAsync"]
    status?: MiaopanCodeClient["session"]["status"]
    messages?: MiaopanCodeClient["session"]["messages"]
    children?: MiaopanCodeClient["session"]["children"]
    permissions?: MiaopanCodeClient["permission"]["list"]
    questions?: MiaopanCodeClient["question"]["list"]
  } = {},
) {
  ...
  const status: MiaopanCodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: MiaopanCodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: MiaopanCodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: MiaopanCodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: MiaopanCodeClient["question"]["list"] = input.questions ?? (() => ok([]))
