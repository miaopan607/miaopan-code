...
// `MIAOPAN_CODE_CONFIG_CONTENT` providing the test provider config inline.
...
describe("miaopan-code run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* miaopanCode.run("say hi")
        miaopanCode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* miaopanCode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        miaopanCode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* miaopanCode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* miaopanCode.run("say hi", { format: "json" })
        miaopanCode.expectExit(result, 0)

        const events = miaopanCode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const result = yield* miaopanCode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* miaopanCode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* miaopanCode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const allowed = yield* miaopanCode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        ...
        const explicitlyDenied = yield* miaopanCode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        miaopanCode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, miaopanCode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* miaopanCode.serve()

        const result = yield* miaopanCode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, miaopanCode }) =>
      Effect.gen(function* () {
        const result = yield* miaopanCode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, miaopanCode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* miaopanCode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
