    ...
    const cases = [
      {
        ...
        expected: "Config file at /tmp/miaopan-code.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/miaopan-code.jsonc", dir: ".miaopanCode", suggestion: "miaopan-code" },
        expected:
          'Directory ".miaopanCode" in /tmp/miaopan-code.jsonc is not valid. Rename the directory to "miaopan-code" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/miaopan-code.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
