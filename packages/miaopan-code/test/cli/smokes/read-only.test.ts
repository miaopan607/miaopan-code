...
describe("miaopanCode read-only commands (smoke)", () => {
  ...
  // MIAOPAN_CODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const r = yield* miaopanCode.spawn(["mcp", "list"])
        miaopanCode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const r = yield* miaopanCode.spawn(["models"])
        miaopanCode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const r = yield* miaopanCode.spawn(["agent", "list"])
        miaopanCode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh MIAOPAN_CODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const r = yield* miaopanCode.spawn(["session", "list"])
        miaopanCode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const r = yield* miaopanCode.spawn(["stats"])
        miaopanCode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under MIAOPAN_CODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ miaopanCode }) =>
      Effect.gen(function* () {
        const r = yield* miaopanCode.spawn(["db", "path"])
        miaopanCode.expectExit(r, 0, "db path")
