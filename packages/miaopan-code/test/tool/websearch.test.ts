...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.MIAOPAN_CODE_WEBSEARCH_PROVIDER
    ...
    try {
      ...
      if (original === undefined) delete process.env.MIAOPAN_CODE_WEBSEARCH_PROVIDER
      else process.env.MIAOPAN_CODE_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for miaopanCode or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.miaopanCode, { exa: false, parallel: false })).toBe(true)
