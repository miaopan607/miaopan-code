...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".miaopanCode")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .miaopanCode/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const miaopanCode = path.join(test.directory, ".miaopanCode")
      const tool = path.join(miaopanCode, "tool")
  ...
  it.instance("ignores non-tool exports in .miaopanCode/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".miaopanCode", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-miaopanCode` was registering a tool
  ...
  it.instance("loads tools from .miaopanCode/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const miaopanCode = path.join(test.directory, ".miaopanCode")
      const tools = path.join(miaopanCode, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".miaopanCode", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const miaopanCode = path.join(test.directory, ".miaopanCode")
        const customTools = path.join(miaopanCode, "tools")
        const plugin = path.join(miaopanCode, "node_modules", "@miaopan-code", "plugin")
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".miaopanCode", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(miaopanCode, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(miaopanCode, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(miaopanCode, "node_modules", "cowsay")
