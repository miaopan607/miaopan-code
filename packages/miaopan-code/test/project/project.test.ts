...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const miaopanCodeFile = path.join(tmp, ".git", "miaopan-code")
      expect(yield* Effect.promise(() => Bun.file(miaopanCodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "miaopan-code")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "miaopan-code")
      const wrongCache = path.join(parentDir, ".git", "miaopan-code")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "miaopan-code")
      const cacheB = path.join(bareB, "miaopan-code")
      const wrongCache = path.join(parentDir, ".git", "miaopan-code")
