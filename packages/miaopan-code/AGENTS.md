...
- Running `bun dev` from `packages/miaopan-code` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s miaopan-code-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt miaopan-code-dev`.
- Stop the session explicitly when done: `tmux kill-session -t miaopan-code-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@miaopan-code/Foo") {}
