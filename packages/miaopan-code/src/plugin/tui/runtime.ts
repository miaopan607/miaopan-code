...
import { hasTheme, upsertTheme } from "@miaopan-code/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@miaopan-code/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@miaopan-code/core/v1/config/plugin"
import { createCommandShim } from "@miaopan-code/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@miaopan-code/tui/plugin/runtime"
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.MIAOPAN_CODE_PURE ? [] : pluginOrigins
    if (Flag.MIAOPAN_CODE_PURE && pluginOrigins.length) {
    }
