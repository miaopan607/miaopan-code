...
import { IdeEvent } from "@miaopan-code/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["MIAOPAN_CODE_CALLER"] === "vscode" || process.env["MIAOPAN_CODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.miaopanCode"], {
    nothrow: true,
  })
