...
const TUI_SCHEMA_URL = "https://raw.githubusercontent.com/miaopan607/miaopan-code/main/schemas/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from miaopan-code.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const miaopanCode = await miaopanCodeFiles(input)
  for (const file of miaopanCode) {
...
async function miaopanCodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "miaopan-code"),
    ...(await Filesystem.findUp(["miaopan-code.json", "miaopan-code.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "miaopan-code"))
  }
  if (Flag.MIAOPAN_CODE_CONFIG) files.push(Flag.MIAOPAN_CODE_CONFIG)
