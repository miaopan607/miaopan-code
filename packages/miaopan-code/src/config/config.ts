...
import { InstallationLocal, InstallationVersion } from "@miaopan-code/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@miaopan-code/core/v1/config/console-state"
import { FSUtil } from "@miaopan-code/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@miaopan-code/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@miaopan-code/core/v1/config/config"
import { RemoteAuthError } from "@miaopan-code/core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@miaopan-code/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["miaopan-code.jsonc", "miaopan-code.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.MIAOPAN_CODE_CONFIG && !Flag.MIAOPAN_CODE_CONFIG_DIR && !Flag.MIAOPAN_CODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "miaopan-code.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "miaopan-code.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://raw.githubusercontent.com/miaopan607/miaopan-code/main/schemas/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "MIAOPAN_CODE_CONFIG_CONTENT") return "local"
        ...
        if (!Flag.MIAOPAN_CODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("miaopan-code", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.miaopan-code/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.MIAOPAN_CODE_CONFIG_CONTENT) {
          const source = "MIAOPAN_CODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.MIAOPAN_CODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
        ...
        if (Flag.MIAOPAN_CODE_PERMISSION) {
          try {
        ...
        if (Flag.MIAOPAN_CODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.MIAOPAN_CODE_DISABLE_PRUNE) {
