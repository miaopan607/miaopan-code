...
type ModelInput = Parameters<MiaopanCodeClient["session"]["prompt"]>[0]["model"]
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachSDK = (dir?: string) => {
        return createMiaopanCodeClient({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(sdk: MiaopanCodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: MiaopanCodeClient, sessionID: string) {
      ...
      async function current(sdk: MiaopanCodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: MiaopanCodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: MiaopanCodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: MiaopanCodeClient) {
        ...
        async function loop(client: MiaopanCodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createMiaopanCodeClient({
        baseUrl: "http://miaopanCode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
