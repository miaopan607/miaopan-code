...
export type Todo = {
  ...
  content: string
  /**
   * 任务的当前状态：pending、in_progress、completed、cancelled
   */
  status: string
  /**
...
export type ProviderConfig = {
  ...
  options?: {
    ...
    setCacheKey?: boolean
    /**
     * 模型上下文协议（MCP）请求的超时时间（毫秒）
     */
    timeout?: number | false
    /**
     * 等待提供商响应标头的超时时间（毫秒）；设为 false 可禁用
     */
...
export type Config = {
  ...
  autoshare?: boolean
  /**
   * 自动更新到最新版本；true 自动更新，false 禁用，notify 显示更新通知
   */
...
export type V2SessionListData = {
  ...
  query?: {
    ...
    subpath?: string
    /**
     * 上一个响应中作为 cursor.previous 或 cursor.next 返回的不透明分页游标。
     */
    cursor?: string
  }
  url: "/api/session"
...
export type V2SessionMessagesData = {
  ...
  query?: {
    ...
    order?: "asc" | "desc"
    /**
     * 上一个响应中作为 cursor.previous 或 cursor.next 返回的不透明分页游标。不能与 order 一起使用。
     */
    cursor?: string
  }
