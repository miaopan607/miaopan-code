...
export const createClient = (config: Config = {}): Client => {
  ...
  const beforeRequest = async (options: RequestOptions) => {
    ...
    // 正文为空时移除 Content-Type 标头，以免发送无效请求
    ...
    if (response.ok) {
      ...
      switch (parseAs) {
        ...
        case "text":
          ...
          break
        case "json": {
          // 某些服务器返回 200，但没有 Content-Length 且正文为空。
          // response.json() 会抛出异常；先读取文本，仅在非空时解析。
