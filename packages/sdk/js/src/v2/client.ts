...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-miaopanCode-directory", "directory"],
    ["x-miaopanCode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-miaopanCode-workspace")
  return next
}

export function createMiaopanCodeClient(
  config?: Config & { directory?: string; experimental_workspaceID?: string; language?: Language },
) {
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-miaopanCode-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html") throw new Error(t(language, "client_unsupported_server"))
  ...
  return new MiaopanCodeClient({ client, language })
}
