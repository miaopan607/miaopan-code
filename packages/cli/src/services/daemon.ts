...
import { createMiaopanCodeClient } from "@miaopan-code/sdk/v2/client"
import { ServerAuth } from "@miaopan-code/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createMiaopanCodeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@miaopan-code/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createMiaopanCodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createMiaopanCodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
