...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      MIAOPAN_CODE_CLI_NAME: `'${binary}'`,
      MIAOPAN_CODE_MODELS_DEV: modelsData,
      MIAOPAN_CODE_CHANNEL: `'${Script.channel}'`,
      MIAOPAN_CODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@miaopan-code/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/miaopan607/miaopan-code.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
