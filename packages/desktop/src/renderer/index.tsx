...
import type { UpdaterState } from "@miaopan-code/app/updater"
...
import "./styles.css"
import { Splash } from "@miaopan-code/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.MIAOPAN_CODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "miaopanCode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__MIAOPAN_CODE__ ??= {}
  const pending = window.__MIAOPAN_CODE__.deepLinks ?? []
  window.__MIAOPAN_CODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `miaopanCode.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://github.com/miaopan607/miaopan-code/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("miaopanCode.global.dat").getItem("language")
