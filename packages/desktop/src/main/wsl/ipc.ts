...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-miaopanCode", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installMiaopanCode(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-miaopanCode", unavailable)
