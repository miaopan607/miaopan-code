...
export async function installWslMiaopanCode(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://github.com/miaopan607/miaopan-code/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslMiaopanCode(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.miaopanCode/bin/miaopanCode" ]; then printf "%s\\n" "$HOME/.miaopanCode/bin/miaopanCode"; fi',
        distro,
        opts,
      )
    ).stdout,
