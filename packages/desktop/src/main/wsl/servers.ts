...
import { expectMiaopanCodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
import { clearWslDistroState, wslServerIdToRestart } from "./policy"
import {
  installWslDistro,
  installWslMiaopanCode,
...
type WslServersControllerOptions = {
  ...
  resolveMiaopanCode?: typeof resolveWslMiaopanCode
...
export function createWslServersController(
  ...
  const setMiaopanCodeCheck = (distro: string, check: WslMiaopanCodeCheck) => {
    setState({
      miaopanCodeChecks: {
        ...state.miaopanCodeChecks,
        [distro]: check,
      },
    })
  }

  const checkMiaopanCode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveMiaopanCode ?? resolveWslMiaopanCode)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return miaopanCodeCheck(distro, resolved, version, appVersion)
  }

  const refreshMiaopanCodeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setMiaopanCodeCheck(distro, await checkMiaopanCode(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const miaopanCodeChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.miaopanCodeChecks[distro])
        .map(async (distro) => [distro, await checkMiaopanCode(distro, opts)] as const),
    )
    if (miaopanCodeChecks.length) {
      setState({ miaopanCodeChecks: { ...state.miaopanCodeChecks, ...Object.fromEntries(miaopanCodeChecks) } })
    }
  }
  ...
  const refreshMiaopanCodeCheckBackground = (id: string, distro: string) => {
    void checkMiaopanCode(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setMiaopanCodeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl miaopanCode check failed", { id, distro, message })
      })
  }

  const refreshMiaopanCodeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkMiaopanCode(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setMiaopanCodeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl miaopanCode check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshMiaopanCodeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installMiaopanCode(name: string) {
      await runJob({ kind: "install-miaopanCode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslMiaopanCode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "MiaopanCode installation failed")
        }
        await refreshMiaopanCodeCheck(name, { signal: abort.signal })
        expectMiaopanCodeVersion(state.miaopanCodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.miaopanCodeChecks, distro) : {}),
      })
    },
...
function miaopanCodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "miaopanCode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "miaopanCode is installed but could not run",
    }
  }
  return {
    distro,
