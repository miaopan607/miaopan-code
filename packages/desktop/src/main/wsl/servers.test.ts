...
let releaseMiaopanCodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectMiaopanCodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectMiaopanCodeVersion("1.14.35", "1.16.2")).toThrow(
    "MiaopanCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating MiaopanCode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.miaopanCode/bin/miaopanCode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, miaopanCodeChecks: {} })
})
...
test("ignores stale background MiaopanCode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseMiaopanCodeResolve = undefined
  ...
  await waitFor(() => !!releaseMiaopanCodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseMiaopanCodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().miaopanCodeChecks).toEqual({})
})

test("ignores stale startup MiaopanCode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking MiaopanCode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const miaopanCode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveMiaopanCode: async (distro) => {
      miaopanCode.push(distro)
      return "/home/me/.miaopanCode/bin/miaopanCode"
    },
  })
  ...
  expect(miaopanCode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(miaopanCode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().miaopanCodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check MiaopanCode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const miaopanCode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(miaopanCode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().miaopanCodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveMiaopanCode: async () => {
      await new Promise<void>((resolve) => {
        releaseMiaopanCodeResolve = resolve
      })
      return "/home/me/.miaopanCode/bin/miaopanCode"
    },
  }
