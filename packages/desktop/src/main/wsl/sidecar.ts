...
import { type WslCommandLine, resolveWslMiaopanCode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const miaopanCode = await resolveWslMiaopanCode(distro)
  if (!miaopanCode) throw new Error(`MiaopanCode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "miaopan-code"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export MIAOPAN_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export MIAOPAN_CODE_CLIENT=desktop",
    `export MIAOPAN_CODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export MIAOPAN_CODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(miaopanCode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
