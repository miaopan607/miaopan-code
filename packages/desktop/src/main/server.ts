...
const SIDECAR_SERVICE_NAME = "miaopanCode server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    MIAOPAN_CODE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    MIAOPAN_CODE_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`miaopanCode:${password}`).toString("base64")
...
function createSidecarEnv(): Record<string, string> {
  ...
  if (!app.isPackaged) env.MIAOPAN_CODE_DISABLE_CHANNEL_DB = "1"
  return env
}
