...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.miaopanCode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `miaopanCode.settings.dat` is special: it maps to the `miaopanCode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"miaopanCode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // miaopanCode.settings.dat → the electron settings store ("miaopanCode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "miaopanCode.global.dat").
  const storeName = filename === "miaopanCode.settings.dat" ? "miaopanCode.settings" : filename
