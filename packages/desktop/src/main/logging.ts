...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `miaopanCode-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "miaopan-code", "log"), join(app.getPath("userData"), "miaopan-code", "log")])]
}
