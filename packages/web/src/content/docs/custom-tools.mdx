---
title: Custom Tools
description: Create tools the LLM can call in miaopan-code.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside miaopan-code's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.miaopan-code/tools/` directory of your project.
- Or globally, by placing them in `~/.config/miaopan-code/tools/`.
...
```ts title=".miaopan-code/tools/database.ts" {1}
...
```ts title=".miaopan-code/tools/math.ts"
import { tool } from "@miaopan-code/plugin"
...
```ts title=".miaopan-code/tools/bash.ts"
import { tool } from "@miaopan-code/plugin"
...
```ts title=".miaopan-code/tools/project.ts" {8}
import { tool } from "@miaopan-code/plugin"
...
```python title=".miaopan-code/tools/add.py"
import sys
...
```ts title=".miaopan-code/tools/python-add.ts" {10}
import { tool } from "@miaopan-code/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".miaopan-code/tools/add.py")
