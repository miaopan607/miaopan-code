...
You can add external tools to miaopan-code using the _Model Context Protocol_, or MCP. miaopan-code supports both local and remote servers.
...
You can define MCP servers in your [miaopan-code Config](https://github.com/miaopan607/miaopan-code/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="miaopan-code.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/miaopan-code` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="miaopan-code.jsonc" {15}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
miaopan-code automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, miaopan-code will:
...
If the server requires authentication, miaopan-code will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `miaopan-code mcp auth <server-name>`.
...
```json title="miaopan-code.json" {7-11}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
miaopan-code mcp auth my-oauth-server
```
...
```bash
miaopan-code mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, miaopan-code will store the tokens securely in `~/.local/share/miaopan-code/mcp-auth.json`.
...
```json title="miaopan-code.json" {7}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "my-api-key-server": {
...
miaopan-code mcp debug my-oauth-server
```
...
Your MCPs are available as tools in miaopan-code, alongside built-in tools. So you can manage them through the miaopan-code config like any other tool.
...
```json title="miaopan-code.json" {14}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="miaopan-code.json" {11, 14-18}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="miaopan-code.json" {4-8}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "sentry": {
...
```bash
miaopan-code mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect miaopan-code to your Sentry account.
...
```json title="miaopan-code.json" {4-7}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "context7": {
...
```json title="miaopan-code.json" {7-9}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "mcp": {
    "context7": {
