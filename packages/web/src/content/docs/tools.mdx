...
Tools allow the LLM to perform actions in your codebase. miaopan-code comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in miaopan-code.
...
```json title="miaopan-code.json" {4}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "permission": {
    "bash": "allow"
...
:::note
This tool is only available when `MIAOPAN_CODE_EXPERIMENTAL_LSP_TOOL=true` (or `MIAOPAN_CODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the miaopan-code provider or when the `MIAOPAN_CODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching miaopan-code:

```bash
MIAOPAN_CODE_ENABLE_EXA=1 miaopan-code
```
