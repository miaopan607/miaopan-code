...
With miaopan-code you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, miaopan-code uses our own `miaopan-code` theme.
...
miaopan-code comes with several built-in themes.
...
- Want miaopan-code to match their terminal's appearance
...
miaopan-code supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/miaopan-code/themes/*.json` or `$XDG_CONFIG_HOME/miaopan-code/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.miaopan-code/themes/*.json`
4. **Current working directory** - Defined in `./.miaopan-code/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/miaopan-code/themes
vim ~/.config/miaopan-code/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .miaopan-code/themes
vim .miaopan-code/themes/my-theme.json
```
...
{
  "$schema": "https://github.com/miaopan607/miaopan-code/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
