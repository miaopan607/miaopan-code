---
title: Providers
description: Using any LLM provider in miaopan-code.
---

import config from "../../../config.mjs"
export const console = config.console

miaopan-code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your miaopan-code config.
...
in `~/.local/share/miaopan-code/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your miaopan-code
config.
...
```json title="miaopan-code.json" {6}
{
...
miaopan-code Zen is a list of models provided by the miaopan-code team that have been
tested and verified to work well with miaopan-code. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with miaopan-code Zen.
:::

1. Run the `/connect` command in the TUI, select `miaopan-code Zen`, and head to [github.com/miaopan607/miaopan-code/auth](https://github.com/miaopan607/miaopan-code/zen).
...
It works like any other provider in miaopan-code and is completely optional to use.

---

## miaopan-code Go

miaopan-code Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the miaopan-code team that have been
tested and verified to work well with miaopan-code.

1. Run the `/connect` command in the TUI, select `miaopan-code Go`, and head to [github.com/miaopan607/miaopan-code/auth](https://github.com/miaopan607/miaopan-code/zen).
...
To use Amazon Bedrock with miaopan-code:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running miaopan-code:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY miaopan-code

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile miaopan-code

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX miaopan-code
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `miaopan-code.json`:

   ```json title="miaopan-code.json"
   {
     "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
     "provider": {
       "amazon-bedrock": {
...
miaopan-code. Anthropic explicitly prohibits this.

Previous versions of miaopan-code came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in miaopan-code with zero setup:
...
You can configure miaopan-code to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="miaopan-code.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for miaopan-code to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX miaopan-code
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX miaopan-code
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your miaopan-code config.
...
miaopan-code supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; miaopan-code uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize miaopan-code. Sign in and approve.

   :::note
   miaopan-code requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, miaopan-code surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — miaopan-code forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in miaopan-code is experimental. Features, configuration, and
behavior may change in future releases.
:::

miaopan-code integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `miaopan-code`, Scopes: `api`
...
to store token in miaopan-code auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
miaopan-code uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock miaopan-code
to only use your own GitLab-hosted instance, add the following to your
`miaopan-code.json` file. It is also recommended to disable session sharing.
...
More documentation on [miaopan-code-gitlab-auth](https://www.npmjs.com/package/miaopan-code-gitlab-auth) homepage.

##### Configuration

Customize through `miaopan-code.json`:
...
When a `duo-workflow-*` model is selected, miaopan-code will:
...
4. Route tool execution requests through miaopan-code's permission-gated tool system
...
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "plugin": ["miaopan-code-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with miaopan-code:
...
To use GMI Cloud with miaopan-code:
...
To use Google Vertex AI with miaopan-code:
...
2. Set the required environment variables:
   ...
   Set them while running miaopan-code.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id miaopan-code
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through miaopan-code, you can always configure it yourself.
...
```jsonc title="~/.config/miaopan-code/miaopan-code.jsonc"
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [miaopan-code-helicone-session](https://github.com/H2Shami/miaopan-code-helicone-session) plugin to automatically log each miaopan-code conversation as a session in Helicone.

```bash
npm install -g miaopan-code-helicone-session
```
...
```json title="miaopan-code.json"
{
  "plugin": ["miaopan-code-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each miaopan-code conversation listed as a separate session.
...
You can configure miaopan-code to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="miaopan-code.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure miaopan-code to use local models through LM Studio.

```json title="miaopan-code.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "lmstudio": {
...
You can configure miaopan-code to use local models through Ollama.

:::tip
Ollama can automatically configure itself for miaopan-code. See the [Ollama integration docs](https://docs.ollama.com/integrations/miaopan-code) for details.
:::

```json title="miaopan-code.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with miaopan-code:
...
3. Copy the API key for use in miaopan-code.
...
6. **Important**: Before using cloud models in miaopan-code, you must pull the model information locally:
...
miaopan-code Zen is a list of tested and verified models provided by the miaopan-code team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>miaopan-code Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **miaopan-code Zen**.

   ```txt
   /connect
   ```

3. Enter your miaopan-code API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your miaopan-code config.
...
5. You can also customize them through your miaopan-code config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' miaopan-code
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group miaopan-code
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with miaopan-code:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=miaopan-code&utm_medium=docs&utm_campaign=cortex-provider).

miaopan-code's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. miaopan-code captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting miaopan-code:
...
The model catalog is provided automatically. A minimal `miaopan-code.json` is all that's needed:
...
You can also customize models through your miaopan-code config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. miaopan-code opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
miaopan-code refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when miaopan-code is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running miaopan-code — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. miaopan-code prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. miaopan-code polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with miaopan-code. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in miaopan-code.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `miaopan-code.json` file in your project directory:

   ```json title="miaopan-code.json" ""myprovider"" {5-15}
   {
     "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
     "provider": {
       "myprovider": {
...
```json title="miaopan-code.json" {9,11,17-20}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow miaopan-code to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `miaopan-code auth list` to see if the credentials
...
2. For custom providers, check the miaopan-code config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your miaopan-code config.
