...
There are two types of agents in miaopan-code; primary agents and subagents.
...
miaopan-code comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
miaopan-code comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

miaopan-code comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into miaopan-code's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `miaopan-code.json` config file:
...
- Global: `~/.config/miaopan-code/agents/`
- Per-project: `.miaopan-code/agents/`

```markdown title="~/.config/miaopan-code/agents/review.md"
---
...
If no temperature is specified, miaopan-code uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global miaopan-code config and the project specific config.
...
The model ID in your miaopan-code config uses the format `provider/model-id`. For example, if you're using [miaopan-code Zen](/docs/zen), you would use `miaopan-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="miaopan-code.json" {3-6,9-12}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "tools": {
    "write": true,
...
```json title="miaopan-code.json" {3-5,8-10}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "permission": {
    "edit": "deny"
...
```json title="miaopan-code.json" {8}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "agent": {
    "build": {
...
```json title="miaopan-code.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `miaopan-code models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/miaopan607/miaopan-code).
:::
...
```markdown title="~/.config/miaopan-code/agents/docs-writer.md"
---
...
```markdown title="~/.config/miaopan-code/agents/security-auditor.md"
---
