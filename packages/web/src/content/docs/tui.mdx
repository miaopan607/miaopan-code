---
title: TUI
description: Using the miaopan-code terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

miaopan-code provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running miaopan-code starts the TUI for the current directory.
...
When using the miaopan-code TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to miaopan-code. Allows you to select from available providers and add their API keys.
...
Exit miaopan-code. _Aliases_: `/quit`, `/q`
...
This is separate from `miaopan-code.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long miaopan-code waits after the leader key. Defaults to `2000`.
...
Use `MIAOPAN_CODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `miaopan-code.default`.
