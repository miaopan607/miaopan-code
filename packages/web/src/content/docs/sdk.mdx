---
title: SDK
description: Type-safe JS client for miaopan-code server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The miaopan-code JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control miaopan-code programmatically.
...
Create an instance of miaopan-code:

```javascript
import { createMiaopanCode } from "@miaopan-code/sdk"

const { client } = await createMiaopanCode()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `miaopan-code.json`, but you can override or add configuration inline:

```javascript
import { createMiaopanCode } from "@miaopan-code/sdk"

const miaopan-code = await createMiaopanCode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${miaopan-code.server.url}`)

miaopan-code.server.close()
```

## Client only

If you already have a running instance of miaopan-code, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@miaopan-code/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*miaopan-code" },
})
