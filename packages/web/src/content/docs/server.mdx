---
title: Server
description: Interact with miaopan-code server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `miaopan-code serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an miaopan-code client can use.
...
```bash
miaopan-code serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `miaopan-code.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
miaopan-code serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `MIAOPAN_CODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `miaopan-code`, or set `MIAOPAN_CODE_SERVER_USERNAME` to override it. This applies to both `miaopan-code serve` and `miaopan-code web`.

```bash
MIAOPAN_CODE_SERVER_PASSWORD=your-password miaopan-code serve
```
...
When you run `miaopan-code` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the miaopan-code server to interact with miaopan-code programmatically.
:::

This architecture lets miaopan-code support multiple clients and allows you to interact with miaopan-code programmatically.

You can run `miaopan-code serve` to start a standalone server. If you have the
miaopan-code TUI running, `miaopan-code serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the miaopan-code [IDE](/docs/ide) plugins.
...
The miaopan-code server exposes the following APIs.
