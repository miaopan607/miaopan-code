---
title: Formatters
description: miaopan-code uses language specific formatters.
---

miaopan-code can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before miaopan-code will run them.

---

## Built-in

miaopan-code comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
When formatters are enabled, miaopan-code will use `prettier` for matching files if your project has `prettier` in `package.json`.

---

## How it works

When miaopan-code writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your miaopan-code config.
...
```json title="miaopan-code.json" {5}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "formatter": {
    "prettier": {
...
```json title="miaopan-code.json" {4-14}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "formatter": {
    "prettier": {
