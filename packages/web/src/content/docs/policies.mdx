---
title: Policies
description: Control which configured resources miaopan-code may use.
---

Policies control whether miaopan-code may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `miaopan-code.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether miaopan-code may use a resource such as an LLM provider.
...
miaopan-code currently supports one policy action:
