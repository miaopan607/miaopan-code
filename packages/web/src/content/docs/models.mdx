...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start miaopan-code.
...
Here are several models that work well with miaopan-code, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
```json title="miaopan-code.json" {3}
{
...
Here the full ID is `provider_id/model_id`. For example, if you're using [miaopan-code Zen](/docs/zen), you would use `miaopan-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="miaopan-code.jsonc" {7-12,19-24}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "openai": {
...
```jsonc title="miaopan-code.jsonc" {6-21}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "miaopan-code": {
...
Many models support multiple variants with different configurations. miaopan-code ships with built-in default variants for popular providers.

### Built-in variants

miaopan-code ships with default variants for many providers:
...
```jsonc title="miaopan-code.jsonc" {7-18}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "provider": {
    "openai": {
...
When miaopan-code starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the miaopan-code config.
