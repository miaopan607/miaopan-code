---
title: LSP Servers
description: miaopan-code integrates with your LSP servers.
---

miaopan-code can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

miaopan-code comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `MIAOPAN_CODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and miaopan-code opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your miaopan-code config.
...
```json title="miaopan-code.json" {5-8}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "lsp": {
    "rust": {
...
```json title="miaopan-code.json" {5-13}
{
  "$schema": "https://github.com/miaopan607/miaopan-code/config.json",
  "lsp": {
    "custom-lsp": {
