...
import { AbsolutePath, Agent, Location, Model, MiaopanCode, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* MiaopanCode.make({ baseUrl: "http://localhost:3000" })
