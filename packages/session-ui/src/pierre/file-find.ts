...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("miaopanCode-find")
  api.delete("miaopanCode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("miaopanCode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("miaopanCode-find", new Highlight(...rest))
    return true
  }
