...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@miaopan-code/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/miaopanCode`)).toBe("path")
    expect(inlineCodeKind(`@miaopan-code/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://github.com/miaopan607/miaopan-code/docs`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/miaopanCode`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://github.com/miaopan607/miaopan-code/docs`)).toBeUndefined()
  })
})
