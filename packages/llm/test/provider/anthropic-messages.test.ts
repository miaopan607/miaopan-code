...
describe("Anthropic Messages route", () => {
  ...
  it.effect("rejects non-text chronological system update content before send", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toContain("Anthropic Messages 的 system 消息目前只支持文本内容")
    }),
  )
  ...
  it.effect("rejects a system update between a local tool call and its result", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toContain("系统更新不能拆分本地工具调用及其工具结果")
    }),
  )
  ...
  it.effect("falls back to a stable default when error payload is absent", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toMatchObject([{ type: "provider-error", message: "Anthropic Messages 流错误" }])
    }),
  )
