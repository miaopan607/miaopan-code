...
User- and model-visible library messages default to Simplified Chinese. Set `language: "en"` on `LLM.request(...)` to keep validation errors, transport diagnostics, stream fallbacks, and generated tool messages in English. Standalone tool dispatch accepts the same value as the third argument to `ToolRuntime.dispatch(tools, call, language)`.
