...
**miaopan-code Client**:
The generated Promise API derived from the public `HttpApi`, consumed internally by the CLI and TUI.
...
- The server's concrete `HttpApi` is authoritative for the generated SDK. Protocol owns endpoint construction and middleware placement; Server supplies concrete handlers and services.
- The Promise SDK returns typed values, preserves declared failures, and keeps transport options separate from domain inputs.
- Promise streaming methods return lazy `AsyncIterable` values and fail explicitly on transport loss; callers own any resubscription policy.
- SDK construction is synchronous and network-free. It requires `baseUrl`, defaults to `globalThis.fetch`, and accepts client-level headers.
- The generated Promise client ships from `@miaopan-code/sdk` and is used internally by the CLI and TUI.
- The SDK preserves Server routing, codecs, handlers, and errors through the public HTTP boundary; it does not host an in-process server.
- A capability intended for CLI and TUI belongs in the authoritative public `HttpApi` and its generated SDK.
- `sessions.events({ sessionID, after })` is a public durable Session event stream. It verifies the Session, replays durable events after the optional aggregate sequence, continues with newly committed durable events, and excludes live-only fragments.
...
- The stable `sessions.list(...)` design returns a **Page** and preserves the existing HTTP `{ data, cursor }` envelope.
...
- `sessions.create(...)` accepts an optional `location`. Omission resolves through the connected miaopan-code instance's default or current location; an explicit value selects a known location.
...
- Keep concrete Location middleware keys in Server while Protocol owns their placement.
...
- Preserve the browser-safe generated SDK surface through its import-boundary tests.
