...
miaopan-code web             # Start server + open web interface
...
1. **First, start the miaopan-code server** (see [Running the API Server](#running-the-api-server) section above)
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/miaopan-code/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
